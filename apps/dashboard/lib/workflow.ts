...
export function createDraftOptions(candidates: Candidate[]): DraftOption[] {
  ...
  return ranked.slice(0, 2).map((candidate, index) => {
    ...
    return {
      ...
      threadText: [
        ...
        ["오늘 적용할 문장:", "", reusable, "", "이렇게 시키면 글보다 먼저 검토 가능한 연구 노트가 나옵니다."].join("\n"),
        [
          "참고해서 볼 만한 것:",
          sourceUrl,
          `- 볼 부분: ${title}에서 연구 작업을 작은 단위로 나누는 방식`,
          "",
...
export function qualityScore(threadText: string) {
  ...
  if (parts.length < 1 || parts.length > 4) {
    score -= 35;
    reasons.push("Thread must have 1-4 parts.");
  }
  parts.forEach((part, index) => {
    if (part.length > 500) {
      score -= 25;
      reasons.push(`Part ${index + 1} exceeds 500 chars.`);
    }
  });
  ...
  if (!/(프롬프트|문장|체크리스트|기준|workflow|claim|evidence|citation|failure mode)/i.test(threadText)) {
    score -= 20;
    reasons.push("Thread needs a reusable work unit.");
  }
  return { score: Math.max(0, score), passed: score >= 85, reasons: reasons.length ? reasons : ["Quality gate passed."] };
}
